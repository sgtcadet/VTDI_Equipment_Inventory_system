...
/******************************************
* FUNCTIONS ADDED (EQuipment) BY: Howard
******************************************/
...
/*
* Function to get the number of pending request
* @return the (int) number of pending request
* @param $connect
*/
function fetch_pending_requests($connect){
	//Query to run
	$query = "
	SELECT COUNT(inventory_request_status) FROM `inventory_request` 
	WHERE inventory_request_status = 'pending'
	";
	$statement = $connect->prepare($query);
	$statement->execute();
...
function count_total_equipment($connect)
{
	$query = "
	SELECT * FROM equipment WHERE equipment_status='available'
	";
	...
	$statement->execute();
	return $statement->rowCount();}
function count_total_request($connect){
	$query = "
...
function count_total_request_approved($connect)
{

	$query = "
	SELECT * FROM inventory_request
	WHERE inventory_request_status = 'approved'
	";
	if($_SESSION['type'] == 'user')
	{
		$query .= 'AND user_id = "'.$_SESSION["user_id"].'"';
	...
	$statement->execute();
	$result = $statement->rowCount();
	return $result .'/'.  count_total_request($connect);
}
function count_total_request_pending($connect){
	$query = "
	SELECT * FROM inventory_request
	WHERE inventory_request_status = 'pending'
	";
...
function count_total_request_closed($connect){
	$query = "
	SELECT * FROM inventory_request
	WHERE inventory_request_status = 'closed'
	";
	if($_SESSION['type'] == 'user')
	{
		$query .= 'AND user_id = "'.$_SESSION["user_id"].'"';
...
/*
* A functon that check if a user has a image
* @param $connect
* @param $user_id
* @return Boolean true if user has a image or false if they dont
* @author: Howard
*/
function userHasImage($connect, $user_id){
	$query = "
	SELECT * FROM user_details
	WHERE user_id = '".$user_id."'
	AND image IS NOT NULL
	";
	//SELECT * FROM user_details WHERE user_id = '1' AND image IS NOT NULL
	$statement = $connect->prepare($query);
	$statement->execute();
	$result = $statement->rowCount();
	if($result >= 1){
...
function getUserImage($connect, $user_id){
	/*
	$query = "
		SELECT image FROM user_details
		WHERE user_id = '".$user_id."'
	";
	*/
	$query = "
